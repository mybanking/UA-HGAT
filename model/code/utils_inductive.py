...
def get_related_nodes_IGP(adj_list,input_adj_avaliable):

    adj = combine_adj_list(adj_list)
    adj = torch.mm(adj,adj)
    avaliable_num = input_adj_avaliable[0][0].shape[0]

    adj_matrix2_avaliable = adj[0:avaliable_num,0:avaliable_num]

    return np.array(adj_matrix2_avaliable)
