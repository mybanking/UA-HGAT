...
def pseudo_labeling(args, input_adj_test, input_features_test, idx_out_test, idx_test, labels, model, itr, idx_map,
                    iter):
    ...
    with torch.no_grad():
        ...
        ###主动学习  不确性估计 最小置信度
        least_confident(max_value, idx_test, idx_map, args, iter)
...
def least_confident(list, idx_test, idx_map, args, iter):
    ...
    a = idx_test[index].cpu().numpy().tolist()

    new_d = {v: k for k, v in idx_map.items()}

    for i in a:
        index_active_learning.append(new_d.get(i))

    pre_iter = iter - 1
    ### 存起来
    path1 = f'{args.out}/active_learning_iteration_{iter}.pkl'
    pre_path = f'{args.out}/active_learning_iteration_{pre_iter}.pkl'
    if iter != 0:
        b = pkl.load(open(pre_path, 'rb')).get("active_learning")
        for i in b:
            index_active_learning.append(i)

    active_learning_dict = {'active_learning': index_active_learning}
    with open(path1, "wb") as f:
        pkl.dump(active_learning_dict, f)
