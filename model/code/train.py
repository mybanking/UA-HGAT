...
for iter in range(0, args.repeat):
    ...
    # file = open('D:\\study\\KGExperiment\\HGAT_1\\HGAT\\model\\code\\model\\agnews.pkl','rb+')
    # model = pkl.load(file)
    # test(0,input_adj_test,input_features_test,idx_out_test,idx_test)
    # pl_loss, pl_acc, pl_acc_pos, total_sel_pos, pl_acc_neg, total_sel_neg, unique_sel_neg, pseudo_label_dict = pseudo_labeling(
    # args, input_adj_test, input_features_test, idx_out_test, idx_test, labels, model, i)
